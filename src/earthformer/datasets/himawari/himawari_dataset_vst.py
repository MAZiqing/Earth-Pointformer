...
class HimawariDataset(data.Dataset):
    ...
    def __init__(self, root, train=True, split=1000, transform=None, target_transform=None,
        ...
        self.in_len = 2
        self.out_len = 2
        self.samples_per_batch = 13 - self.in_len - self.out_len + 1

    def __getitem__(self, index):
        batch_num = index // self.samples_per_batch
        sid = batch_num % self.samples_per_batch
        if self.train:
            seq, target = self.train_data[batch_num, sid: sid+self.in_len], \
                          self.train_data[batch_num, sid+self.in_len: sid+self.in_len+self.out_len]
        else:
            seq, target = self.test_data[batch_num, sid: sid+self.in_len], \
                          self.test_data[batch_num, sid+self.in_len: sid+self.in_len+self.out_len]
    ...
    def __len__(self):
        if self.train:
            return (len(self.train_data) - 3) * self.samples_per_batch
        else:
            return (len(self.train_data) - 3) * self.samples_per_batch
