...
class ERA5DataModule(pl.LightningDataModule):
    ...
    def prepare_data(self):
        ...
        ERA5Dataset(self.root, train=True, download=True)
        ERA5Dataset(self.root, train=False, download=True)
    ...
    def setup(self, stage: Optional[str] = None):
        if stage == "fit" or stage is None:
            train_val_data = ERA5Dataset(self.root, train=True)
        ...
        if stage == "test" or stage is None:
            self.moving_mnist_test = ERA5Dataset(self.root, train=False)

        if stage == "predict" or stage is None:
            self.moving_mnist_predict = ERA5Dataset(self.root, train=False)
